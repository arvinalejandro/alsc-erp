
<div id="content_body">    
    
    <?=$_VIEW['side_nav']?>
       
    <!---------- MAIN CONTENT ----------->
    <div id="side_listings" style="margin-left:180px;">
    
    	
    	<div class="mar_custom" style="border:10px solid #eaeaea; border-bottom:10px solid #eaeaea; border-top:0; padding-bottom: 5px;" id="controls">
            
            <b class="font_size_18">
                Module - <?=$_VIEW['module_name']?>
            </b>

            <label></label>
        </div> 
    	
        <!---------- CONTENT CONTROLLERS ----------->        
         
        <div style="overflow: auto;" id="_right_content_">  <!--set max-height by making it dynamic -->
           
            <div class="mar_custom" id="client_profile">
                <table class="mar_standard" width="98%" cellpadding="5" cellspacing="0" border="0">
                <thead>
                    <tr>                    	
                        <td>
                            Log Description                   
                        </td>
                        <td>
                        	Date
                        </td>                        
                                             
                    </tr>
                </thead>
                <tbody>
                	<tr class="">                    	
                        <td>
                            <div class="details">
                            	<b>Juan dela Cruz</b> tried accessing <b>Finance - Billing</b>
                            </div>
                        </td>                                          
                        <td>
                            <div class="details">
                               06/30/2014 2:38 PM
                            </div>
                        </td>                                          
                    </tr>      
                    <tr class="">                    	
                        <td>
                            <div class="details">
                            	Added <b>Mizel Delos Santos</b> as user
                            </div>
                        </td>                                          
                        <td>
                            <div class="details">
                               05/25/2014 2:35 PM
                            </div>
                        </td>                                          
                    </tr>      
                    <tr class="">                    	
                        <td>
                            <div class="details">
                            	<b>Mizel Delos Santos</b> changed the value of System Variable - Surcharge divisor from <b>365</b> to <b>265</b>
                            </div>
                        </td>                                          
                        <td>
                            <div class="details">
                               06/30/2014 9:14 PM
                            </div>
                        </td>                                          
                    </tr>
                    <tr class="">                    	
                        <td>
                            <div class="details">
                            Mizel Delos Santos Added <b>Arnie Alejandro</b> as user
                            </div>
                        </td>                                          
                        <td>
                            <div class="details">
                               06/15/2014 8:45 AM
                            </div>
                        </td>                                          
                    </tr>
                    <tr class="">                    	
                        <td>
                            <div class="details">
                            	<b>Arnie Alejandro</b> generated a report : <b>Agent Prod. Report</b>
                            </div>
                        </td>                                          
                        <td>
                            <div class="details">
                               05/25/2014 4:12 PM
                            </div>
                        </td>                                          
                    </tr>
                    <tr class="">                    	
                        <td>
                            <div class="details">
                            	<b>Juan dela Cruz</b> tried accessing <b>Finance - Billing</b>
                            </div>
                        </td>                                          
                        <td>
                            <div class="details">
                               06/30/2014 2:38 PM
                            </div>
                        </td>                                          
                    </tr>      
                    <tr class="">                    	
                        <td>
                            <div class="details">
                            	Added <b>Mizel Delos Santos</b> as user
                            </div>
                        </td>                                          
                        <td>
                            <div class="details">
                               05/25/2014 2:35 PM
                            </div>
                        </td>                                          
                    </tr>      
                    <tr class="">                    	
                        <td>
                            <div class="details">
                            	<b>Mizel Delos Santos</b> changed the value of System Variable - Surcharge divisor from <b>365</b> to <b>265</b>
                            </div>
                        </td>                                          
                        <td>
                            <div class="details">
                               06/30/2014 9:14 PM
                            </div>
                        </td>                                          
                    </tr>
                    <tr class="">                    	
                        <td>
                            <div class="details">
                            	<b>Juan dela Cruz</b> tried accessing <b>Finance - Billing</b>
                            </div>
                        </td>                                          
                        <td>
                            <div class="details">
                               06/30/2014 2:38 PM
                            </div>
                        </td>                                          
                    </tr>      
                    <tr class="">                    	
                        <td>
                            <div class="details">
                            	Added <b>Mizel Delos Santos</b> as user
                            </div>
                        </td>                                          
                        <td>
                            <div class="details">
                               05/25/2014 2:35 PM
                            </div>
                        </td>                                          
                    </tr>      
                    <tr class="">                    	
                        <td>
                            <div class="details">
                            	<b>Mizel Delos Santos</b> changed the value of System Variable - Surcharge divisor from <b>365</b> to <b>265</b>
                            </div>
                        </td>                                          
                        <td>
                            <div class="details">
                               06/30/2014 9:14 PM
                            </div>
                        </td>                                          
                    </tr>
                    <tr class="">                    	
                        <td>
                            <div class="details">
                            Mizel Delos Santos Added <b>Arnie Alejandro</b> as user
                            </div>
                        </td>                                          
                        <td>
                            <div class="details">
                               06/15/2014 8:45 AM
                            </div>
                        </td>                                          
                    </tr>
                    <tr class="">                    	
                        <td>
                            <div class="details">
                            	<b>Arnie Alejandro</b> generated a report : <b>Agent Prod. Report</b>
                            </div>
                        </td>                                          
                        <td>
                            <div class="details">
                               05/25/2014 4:12 PM
                            </div>
                        </td>                                          
                    </tr>
                    <tr class="">                    	
                        <td>
                            <div class="details">
                            Mizel Delos Santos Added <b>Arnie Alejandro</b> as user
                            </div>
                        </td>                                          
                        <td>
                            <div class="details">
                               06/15/2014 8:45 AM
                            </div>
                        </td>                                          
                    </tr>
                    <tr class="">                    	
                        <td>
                            <div class="details">
                            	<b>Arnie Alejandro</b> generated a report : <b>Agent Prod. Report</b>
                            </div>
                        </td>                                          
                        <td>
                            <div class="details">
                               05/25/2014 4:12 PM
                            </div>
                        </td>                                          
                    </tr>
                    
                                  
                                 
                </tbody>  
            </table>
            <label></label>
            </div>  
        </div>    
    </div>
    <label></label>
</div>


