<?php
class model_user_payroll_cutoff
{	
	
	public function __construct()
	{
		$this->table_name		=	'user_payroll_cutoff';
		$this->message_insert	=	'A new entry has been successfully added.';
	}
	
# Data Select

	
	public function select($id,$filter = '')
	{		
		$id		=	string_clean_number($id);		
		$query	=	"SELECT 
						
						* 		
					
					FROM 
					
					user_payroll_cutoff
					
						
					
					WHERE 
					
					user_payroll_cutoff_id = {$id}
					";							
		$rows	=	sql_select($query);	
		$list	=	$rows[0];
		return $list;
	}
	
	
	
	
	public function select_by_cutoff($id,$filter = '')
	{		
		$id		=	string_clean_number($id);		
		$query	=	"SELECT 
						
						* 		
					
					FROM 
					
					user_payroll_cutoff
					
					INNER JOIN payroll_cutoff ON payroll_cutoff.payroll_cutoff_id = user_payroll_cutoff.payroll_cutoff_id
					
					WHERE 
					
					user_payroll_cutoff.payroll_cutoff_id = {$id}
					";							
		$rows	=	sql_select($query);	
		return $rows;
	}
	
	
	
	
	
	
	public function select_by_user_cutoff($id,$user_id,$filter = '')
	{		
		$id		=	string_clean_number($id);		
		$query	=	"SELECT 
						
						* 		
					
					FROM 
					
					user_payroll_cutoff
					
					
					INNER JOIN payroll_cutoff ON payroll_cutoff.payroll_cutoff_id = user_payroll_cutoff.payroll_cutoff_id
					
					
					WHERE 
					
					user_payroll_cutoff.payroll_cutoff_id = {$id}
					
					AND
					
					user_id = {$user_id}
					";							
		$rows	=	sql_select($query);	 
		//hash_dump($query,1);
		return $rows[0];
	}
	
	
	public function select_by_user_cutoff_report($id,$user_id,$filter = '')
	{		
		$id		=	string_clean_number($id);		
		$query	=	"SELECT 
						
						* 		
					
					FROM 
					
					user_payroll_cutoff
					
					INNER JOIN payroll_tax ON payroll_tax.payroll_tax_id = user_payroll_cutoff.payroll_tax_id
					
					WHERE 
					
					user_payroll_cutoff.payroll_cutoff_id = {$id}
					
					AND
					
					user_id = {$user_id}
					";							
		$rows	=	sql_select($query);	 
		//hash_dump($query,1);
		return $rows[0];
	}
	
	
	public function select_by_user($user_id,$filter = '')
	{		
		$id		=	string_clean_number($id);		
		$query	=	"SELECT 
						
						*, (SELECT COUNT(payroll_attendance_id) FROM payroll_attendance WHERE payroll_attendance.payroll_cutoff_id = payroll_cutoff.payroll_cutoff_id) AS stamp_count	 		
					
					FROM 
					
					user_payroll_cutoff
					
					INNER JOIN payroll_cutoff ON payroll_cutoff.payroll_cutoff_id = user_payroll_cutoff.payroll_cutoff_id
					
					INNER JOIN payroll_tax ON payroll_tax.payroll_tax_id = user_payroll_cutoff.payroll_tax_id
					
					INNER JOIN sysglobal_payroll ON sysglobal_payroll.sysglobal_payroll_id = payroll_cutoff.sysglobal_payroll_id
					
					INNER JOIN option_payroll_status ON option_payroll_status.option_payroll_status_id = payroll_cutoff.option_payroll_status_id
					
					WHERE 
					
					user_id = {$user_id}
					
					AND
					
					payroll_cutoff.option_payroll_status_id = 'ready'
					
					AND sysglobal_payroll.is_active = 1
					
					
					";							
		$rows	=	sql_select($query);
		//hash_dump($rows,1);	
		# Template
		$template_row		=	'payroll/template/row.payslip';
		$template_row		=	view_get_template($template_row);
		$template_row_empty	=	'payroll/template/row.empty_cutoff';
		$template_row_empty	=	view_get_template($template_row_empty);
		
		if(count($rows))
		{
			foreach($rows as $row)
			{		
                $row['payroll_cutoff_date_start']    			=  string_date_day_enclosed($row['payroll_cutoff_date_start']);
                $row['payroll_cutoff_date_end']    				=  string_date_day_enclosed($row['payroll_cutoff_date_end']);
                $row['payroll_cutoff_approver']    				=  ($row['payroll_cutoff_approver']) ? $row['payroll_cutoff_approver'] : 'N/A';
                $row['days_count']    							=  ($row['stamp_count']) ? $row['stamp_count'] : 'N/A';
				$list				   						   .=	view_populate($row, $template_row);
			}			
		}
		else
		{
			$list	=	$template_row_empty;
		}
		return $list;
	}
	
	
	
	
	public function select_by_cutoff_list($c_id,$filter = '')
	{		
		$id		=	string_clean_number($id);		
		$query	=	"SELECT 
						
						* 		
					
					FROM 
					
					user_payroll_cutoff
					
					INNER JOIN user ON user.user_id = user_payroll_cutoff.user_id
					
					INNER JOIN payroll_tax ON payroll_tax.payroll_tax_id = user_payroll_cutoff.payroll_tax_id
					
					INNER JOIN option_user_job_title ON option_user_job_title.option_user_job_title_id = user.option_user_job_title_id
					
					INNER JOIN option_department ON option_department.option_department_id = user.option_department_id
					
					INNER JOIN payroll_cutoff ON payroll_cutoff.payroll_cutoff_id = user_payroll_cutoff.payroll_cutoff_id
					
					INNER JOIN sysglobal_payroll ON sysglobal_payroll.sysglobal_payroll_id = payroll_cutoff.sysglobal_payroll_id
					
					INNER JOIN option_payroll_status ON option_payroll_status.option_payroll_status_id = payroll_cutoff.option_payroll_status_id
					
					WHERE 
					
					user_payroll_cutoff.payroll_cutoff_id = {$c_id}
					
					
					
					
					";							
		$rows	=	sql_select($query);	
		# Template
		$template_row		=	'payroll/template/row.cutoff_payslip';
		$template_row		=	view_get_template($template_row);
		$template_row_empty	=	'payroll/template/row.empty_cutoff_payslip';
		$template_row_empty	=	view_get_template($template_row_empty);
		if(count($rows))
		{
			foreach($rows as $row)
			{		
                $row['salary_gross_total']    					=  string_amount($row['salary_gross_total']);
                $row['salary_net']    							=  string_amount($row['salary_net']);
                $row['deduction_tax']    						=  string_amount($row['deduction_tax']);
				$list				   						   .=	view_populate($row, $template_row);
			}			
		}
		else
		{
			$list	=	$template_row_empty;
		}
		return $list;
	}
	
	
   
	
	
# Modify
	
	
	public function insert($post)
	{
		$data														=	sql_parse_input('user_payroll_cutoff', $post);	
		$sql														=	sql_insert('user_payroll_cutoff', $data, 'user_payroll_cutoff_id');			
			
		$return['result']											=	$sql['result'];
		$return['data']												=	$sql['data'];
		$return['message']											=	$sql['message'];	
		//hash_dump($sql,1);		
		return $return;
	}
	
	public function update($post, $id)
	{
		$data					=	sql_parse_input('user_payroll_cutoff', $post);			
									sql_update($this->table_name, 'user_payroll_cutoff_id', $id, $data);	
		$return['result']		=	true;
		$return['data']			=	'';
		$return['message']		=	$this->message_update;
		
	}
	
	public function delete_entry($id)
	{
		sql_delete('user_payroll_cutoff', 'user_payroll_cutoff_id', $id);
	}
	
	
	 
	
}
 