<?php 
class edp_inventory_block
{
    
    public function __construct()
    {    	
       $this->controller_id = 'edp_inventory_block';
    ...
    {      	
       ...
       # $data['option_user_status_bullet']		=	($data['option_user_status_id'] == 1) ? 'paid' : 'attention'; 
        ...
        $side_nav['profile.class']				=	'bold';         	
       	...
       	$data['side_nav']						=	view_get_template($parent->controller_id.'/template/side.block', $side_nav);    
        # VIEW - header      	       	
        $data[$this->controller_id]				=	'<i></i>';
        $data[$this->controller_id.'_class']	=	'class="active"';       
        # VIEW
        $parent->_view('block.profile', $data);        
    }   
